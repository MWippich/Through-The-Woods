Shader "Unlit/windowDrops"
{
    Properties
    {
        ...
        _Blur("Window Blur", range(0,1)) = 0.05
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 Layer(float2 UV, float t){
                ...
                float n = N21(id); // Pseudo random number between [0, 1]
                t += n * 6.2831;   // Multiply the phase so we get full randomness over sin(t). 

                // Add dropspeed and wiggles. 
                float w = UV.y * 10;
                float x = (n-.5)*.8;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Here we can add more rain:
                float3 drops = Layer(i.uv, t);
                drops += Layer(i.uv * 1.23 + 7.54, t);
                drops += Layer(i.uv*1.35 *1.54, t);
                drops += Layer(i.uv*1.57-7.54, t);

                // Handle problem if we are far away from the window:
                // It normalizes UV space based on where the camera is I think:
                float fade = 1 - saturate(fwidth(i.uv) * 50);   // Saturate is like clammping from: 0, 1.


                // Add blur effect and lessen it for the drops trail. 
                float blur = _Blur*7 * (1 - drops.z * fade);
                col = tex2Dlod(_MainTex, float4(i.uv + drops.xy *_Distortion, 0, blur));
                
                // Costy operation for a blur:
                float2 projUv = i.grabUv.xy / i.grabUv.w;
                projUv += drops.xy * _Distortion * fade;
                blur *= .01;

                // Blur (Basically using a box filter sampling with sin)
